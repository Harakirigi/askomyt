...
import { Mail, Phone, Github, Linkedin, ArrowLeft, SendHorizonal, Send } from 'lucide-react';
...
const ContactMeSection: React.FC<ContactMeProps> = ({ backButton }) => {
    ...
    return (
        <>
            <section className="flex items-center self-center justify-center bg-black w-dvw min-h-dvh">
                <main className="flex flex-col max-w-[600px] min-w-[350px] py-8 px-4">
                    {backButton && <Button data-aos="fade-down" variant='accentOutline' styles="group mb-6" onClick={() => window.location.href = '/'} icon={<ArrowLeft width={20} className='transition-transform duration-300 ease-out group-hover:-translate-x-[6px]' />}>Back</Button>}
                    ...
                    <form className="flex flex-col" onSubmit={handleSubmit} data-aos="fade-up">
                        ...
                        <Input
                            ...
                            name="message"
                        />
                        <Button variant="accent" styles='mt-4 group' type="submit" icon={<SendHorizonal width={20} className='transition-transform duration-300 ease-out group-hover:translate-x-[6px]' />}>{t('send')}</Button>                    </form>
