            ...
            <AnimatePresence>
                {shouldShowTooltip && (
                    <motion.div
                        ...
                        className={`absolute z-50 min-w-max px-3 py-1.5 text-xs text-white bg-black rounded-md shadow-lg pointer-events-none ${positionClasses[position]}`}
                        style={style}
                    >
                        {content}
                        <div
                            className={`absolute w-2 h-2 bg-black rotate-45 ${position === 'top' ? 'bottom-[-3px] left-1/2 -translate-x-1/2' :
